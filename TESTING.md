...
## Полномасштабное тестирование с Docker Compose

Для проверки **всех реальных подключений** (nginx, FastAPI, БД, синхронизация) используйте Docker Compose. Это симулирует production-окружение и позволяет провести end-to-end (E2E) тестирование.

### Когда использовать:
- **Проверка инфраструктуры**: Nginx проксирует запросы, API обрабатывает реальные HTTP-запросы, БД хранит данные.
- **Интеграция компонентов**: Тестирование взаимодействия между сервисами (веб-интерфейс, API, БД).
- **E2E-тесты**: Ручное тестирование через браузер или API-клиенты (Postman, curl).
- **Не для unit/integration**: Локальные тесты с mock быстрее и не требуют Docker.
...
2. **Проверить подключения**:
   - **Веб-интерфейс**: Откройте `https://localhost` в браузере.
   - **API**: Отправьте запросы на `https://localhost/api/v1/rules` (используйте curl или Postman).
   - **Синхронизация**: Протестируйте `/api/v1/rules/sync` с тестовыми данными.
...
### Примеры E2E-тестов:
- Создать папку через веб-интерфейс и проверить в БД.
- Синхронизировать правила и убедиться, что они появились в API.
- Проверить SSL-сертификаты и nginx-прокси.
